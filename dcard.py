...
def monitor():
    find_status = False
    pic_status = False
    store_file = '/Users/shark_jhang/Documents/dcard.txt'
    txt = open(store_file, 'a', encoding='UTF-8')
    ...
    if dcard.status_code == requests.codes.ok:
        ...
        for i in range(100):
            try:
                if any(
                    key_word in dcard_contents[i]['title'] + dcard_contents[i]['excerpt'] + dcard_contents[i]['school']
                    for key_word in key_words
                ):
                    find_status = True
                    txt_read = open(store_file, 'r', encoding='UTF-8')
                    if "文章ID：%s" % (str(dcard_contents[i]['id'])) not in txt_read.read():
                        txt.write('\n')
                        txt.write('校名/卡稱：')
                        txt.write(dcard_contents[i]['school'])
                        txt.write('\n')
                        if not dcard_contents[i]['anonymousDepartment']:
                            txt.write('系名：')
                        ...
                        txt.write('\n')
                        txt.write('\n')
                        msg = f"\n校名/卡稱：{dcard_contents[i]['school']}\n系名：{dcard_contents[i]['department'] if not dcard_contents[i]['anonymousDepartment'] else ''}\n文章網址：http://dcard.tw/f/whysoserious/p/{dcard_contents[i]['id']}\n標題：{dcard_contents[i]['title']}\n內文：{dcard_contents[i]['excerpt']}{image_url if pic_status else ''}"
    ...
    while n < number:
        ...
        if dcard.status_code == requests.codes.ok:
            ...
            for i in range(100):
                ...
                try:
                    if department:
                        if dcard_contents[i]['school'] == "%s" % (school) and dcard_contents[i][
                            'department'
                        ] == "%s" % (department):
                            find_status = True
                            txt.write('校名：')
